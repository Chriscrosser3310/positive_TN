...
npz_name = f'{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}_{sys.argv[4]}'
npz_directory = script_directory + f'/{npz_name}.npz'
with open(npz_directory, 'a+') as f:
    pass
print(npz_directory)

avg_table, std_table = avg_entropy_nplist(nlist, plist, bdim=d, repeat=repeat, mode="all_one", prt=True, filename=npz_name)
