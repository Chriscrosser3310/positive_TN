...
if __name__ == "__main__":
    ...
    avg_table, std_table = avg_entropy_nplist(nlist, plist, bdim=d, repeat=100, mode="all_one", prt=True)

    with open('all_one_avg_table.npy', 'wb') as f:
        np.save(f, avg_table)
    with open('all_one_std_table.npy', 'wb') as f:
        np.save(f, std_table)

    fig = plt.figure(figsize=(8, 4), alpha=0)
    fig.patch.set_alpha(1)
    #plt.plot(plist * d**3, avg_table_psd[0, :])
    plt.errorbar(plist * d**3, avg_table[0, :], yerr=std_table[0, :])
